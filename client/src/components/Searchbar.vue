<template>
  <div :class="styling">
    <div class="bg-white text-black w-11/12 m-5 rounded font-bold lexend">
      <input
        type="text"
        v-model="search"
        @keyup="filterData()"
        ...
        >
          <a :href="'/courses/view/' + course._id">
            <div class="flex bg-gray-100 p-3 cursor-pointer border">
              <img
                :src="schoolData(course.school_id).url"
                class="w-12 h-12 rounded-full mr-3"
              />
              <h1 class="text-left mr-2 font-bold">
                {{ course.name }} <br />
                <h1 class="mr-2 text-gray-600 font-semibold">
                  {{ schoolData(course.school_id).name }}
                </h1>
              </h1>
            </div>
          </a>
...
-->
        <div class="bg-gray-300 p-2 text-left">
          ...
          <a href="/schools/add" class="underline">here</a>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    filterData() {
      ...
      this.schoolResults = schoolResults.sort((a, b) =>
